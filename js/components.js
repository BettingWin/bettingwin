((window, document) => {
  ...
  const betItem = {
    ...
    methods: {
      ...
      load: function(item){
        this.loading = true
        tezbridge({method: 'get_contract_info', contract: item.contract})
        .then(x => {
          ...
          item.options = {}
          unpair(storage, 1, 1, 0, 1, 1, 1).args.forEach((x, index) => {
          ...
          unpair(storage, 1, 1, 1, 1, 1, 0).args.forEach(x => {
          ...
          item.bets = {}
          unpair(storage, 1, 1, 1, 1, 1, 1, 0).args.forEach(x => {
            const odd_index = x.args[0].int
            const bet_lst = x.args[1].args.map(x => {
          ...
          item.reports = {}
          unpair(storage, 1, 1, 1, 1, 1, 1, 1, 0).args.forEach(x => {
          ...
          item.distribution = {}
          unpair(storage, 1, 1, 1, 1, 1, 1, 1, 1).args.forEach(x => {
  ...
  const BTWApp = new Vue({
    ...
    methods: {
      ...
      submit_create_bet: function() {
        ...
        this.tezbridge({method: 'originate', amount: 2.01, script: Object.assign({storage: init}, bet_main_script)})
        .then(x => {
          return this.tezbridge({
            method: 'transfer', 
            amount: 0, 
            destination: contracts_xtz.bet_list.contract,
            parameters: window.BTW.parameter.add_to_bet_list({name: this.param.create_bet.info.name, bet_contract: x.contracts[0]})
          })
        })
        .catch(err => {
          if (err) {
      ...
      get_bet_list: function() {
        this.tezbridge({method: 'get_contract_info', contract: contracts_xtz.bet_list.contract})
        .then(x => {
          const lst =  x.script.storage.args[1].args.map(x => ({contract: x.args[1].string, name: x.args[0].string}))
